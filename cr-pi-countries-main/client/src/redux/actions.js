...
export const filterByActivityName = (name) => {
  return async function (dispatch) {
    let filtredById = await axios.get(
      "http://localhost:3001/countries/cbid/" + name
    );
    dispatch({ type: FILTER_BY_ACTIVITY_NAME, payload: filtredById.data });
  };
};

export const filterByContinent = (continent) => {
  return async function (dispatch) {
    let filtredByCont = await axios.get(
      "http://localhost:3001/countries/cbc/" + continent
    );
